...
namespace spritelib
{
	...
	public:
		...
		*/
		...
		/* init
			- gamename is the title that will appear on the titlebar of the window
			- size is a Vector3 of the dimensions of the window that's being created
			- fullscreen is a boolean, which toggles the game in and out of fullscreen.
		*/
		//Window& init(std::string a_gameName, math::Vector3 a_size, bool a_fullscreen);
		/* update
		...
		*/
		...
		/* set_ortho_matrix
			- set the position and size of the screen that sprites render onto. This can be independant from the window position and size
		*/
		Window& set_ortho_matrix(int a_x, int a_y, unsigned int a_width, unsigned int a_height);
		/* set_screen_size
			- set the size of the screen that sprites render onto. This can be independant from the window size
		*/
		//Window& set_screen_size(math::Vector3 a_size);
		/* set_size
			- width/height are the dimensions of the window that will be applied
		*/
		Window& set_size(int a_width, int a_height);
		/* set_size
			- size is a Vector3 of the dimensions of the window that will be applied
		*/
		//Window& set_size(math::Vector3 a_size);
		/* set_title
			- gamename is the title that will appear on the titlebar of the window
		*/
		...
		*/
		...
		/* set_mouse_callback
			...
			- parameter 2 is a Vector3 of the mouse coordiantes (x and y)
			- parameter 3 is the event
		*/
		Window& set_mouse_callback(std::function<void(Button, math::Vector3, EventType)> a_mouseFunction);
		...
		*/
		...
		/* get_size
			- returns the size of the window as a Vector3 of x and y pixels
			- getScreenSize is a boolean. if true, it returns the screen size, otherwise it returns the physical window size
		*/
		math::Vector3 get_size(bool a_getScreenSize = false);
	...
	private:
		...
		std::function<void(Button, math::Vector3, EventType)> m_mouseFunction;
