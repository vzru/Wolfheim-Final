...
namespace spritelib
{
	...
	*/
	...
	/* set_sprite_frame_size
		- size is a Vector3 that will tell the program how big your sprite is on the spritesheet (in pixels)
	*/
	/*Sprite & Sprite::set_sprite_frame_size(math::Vector3 a_size, bool a_setScale)
	{
		//m_frameSize = a_size;
		m_frameSize = { a_size.x, a_size.y };
		if (a_setScale)
		{
			m_transformationUpdated = true;
			m_scale = a_size;
	...
	*/
	...
	/* push_frame_row
		- animationID is a string that you use to set which animation you're using later on
		- start is a Vector3 of the are the coordinates of the very first frame
		- spacing is a Vector3 of the pixel offsets in between each frame
		- numFrames are the number of frames in the row you're adding
	*/
	/*Sprite & Sprite::push_frame_row(std::string a_animationID, math::Vector3 a_start, math::Vector3 a_spacing, unsigned int a_numFrames)
	{
		for (unsigned int i = 0; i < a_numFrames; i++)
		{
			push_frame(a_animationID, a_spacing.multiply(i).add(a_start));
		}
	...
	*/
	...
	/* push_frame
		- animationID is a string that you use to set which animation you're using later on
		- pos is a Vector3 of the coordinates of the frame (bottom left). Be sure to call set_sprite_frame_size() first
	*/
	/*Sprite & Sprite::push_frame(std::string a_animationID, math::Vector3 a_pos)
	{
		m_animations[a_animationID].push_back({ a_pos.x, a_pos.y });
		return *this;
	}*/
	...
	*/
	...
	/* set_center
		- center is a Vector3 of the coordinates of the pivot
	*/
	/*Sprite & Sprite::set_center(math::Vector3 a_center)
	{
		m_transformationUpdated = true;
		m_center = a_center;
		return *this;
	...
	*/
	Sprite & Sprite::draw()
	{
		...
		if (m_transformationUpdated == true)
		{
			...
			m_modelMatrix = m_modelMatrix.multiply(math::Matrix4x4::scale(m_scale.multiply({m_flip ? -1.0f : +1.0f, +1.0f})));
	...
	*/
	...
	/* set_position
		- pos is a Vector3 of the coordinates on the screen where the sprite will be (from the center)
	*/
	Sprite & Sprite::set_position(math::Vector3 a_pos)
	{
		m_transformationUpdated = true;
		m_position = a_pos;
		return *this;
	...
	*/
	...
	/* set_scale
		- size is a Vector3 that will scale your sprite.
	*/
	/*Sprite & Sprite::set_scale(math::Vector3 a_size)
	{
		m_transformationUpdated = true;
		m_scale = a_size;
		return *this;
	...
	*/
	/*void Sprite::Sort(std::vector<Sprite*>& a_sprites)
	{
